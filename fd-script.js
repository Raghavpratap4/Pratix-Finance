...
// Show notification function for FD calculator
function showFDNotification(message, type = 'info') {
    // Remove existing notifications
    const existingNotifications = document.querySelectorAll('.fd-notification');
    existingNotifications.forEach(notif => notif.remove());

    const notification = document.createElement('div');
    notification.className = `fd-notification ${type}`;
...
// Validation functions for FD calculator
function validateFDInputs() {
    ...
    if (principal < 10000) {
        errors.push('Minimum FD amount should be ₹10,000');
    }
    if (principal > 50000000) {
        errors.push('Maximum FD amount is ₹5 crores for this calculator');
    }
    ...
    if (rate < 1) {
        errors.push('Interest rate should be at least 1%');
    }
    if (rate > 15) {
    ...
    if (!tenure || tenure <= 0) {
        errors.push('Please enter a valid tenure');
    }
    if (tenure < 1) {
        errors.push('Minimum FD tenure is 1 year');
    }
    if (tenure > 10) {
        errors.push('Maximum FD tenure is 10 years for this calculator');
    }

    if (!frequency) {
        errors.push('Please select compounding frequency');
    }

    return errors;
}

// FD Calculation with comprehensive validation
function calculateFD() {
    try {
        // Validate inputs
        const errors = validateFDInputs();
        if (errors.length > 0) {
            showFDNotification(errors[0], 'error');
            return;
        }
        ...
        const tenure = parseFloat(document.getElementById('tenureInput').value);
        const frequency = parseInt(document.getElementById('compoundingFrequency').value);
        const fdType = document.querySelector('input[name="fdType"]:checked')?.value || 'regular';

        // Show loading state
        const calculateBtn = document.getElementById('calculateFD');
        const originalText = calculateBtn.innerHTML;
        calculateBtn.innerHTML = '<span class="btn-text">Calculating...</span>';
        calculateBtn.disabled = true;

        setTimeout(() => {
            // Adjust tenure for tax saver FD
            let adjustedTenure = tenure;
            if (fdType === 'taxsaver' && tenure < 5) {
                adjustedTenure = 5;
                showFDNotification('Tax Saver FD has minimum 5-year lock-in period', 'warning');
            }

            // Calculate compound interest
            const rateDecimal = rate / 100;
            const maturityAmount = principal * Math.pow((1 + rateDecimal / frequency), frequency * adjustedTenure);
            const totalInterest = maturityAmount - principal;
            const effectiveYield = ((maturityAmount / principal) ** (1 / adjustedTenure) - 1) * 100;

            // Validate calculation results
            if (isNaN(maturityAmount) || isNaN(totalInterest) || maturityAmount <= principal) {
            ...
            showFDNotification('FD calculation completed successfully!', 'success');
        ...
        showFDNotification('Error in calculation. Please check your inputs.', 'error');

        // Restore button
        const calculateBtn = document.getElementById('calculateFD');
        calculateBtn.innerHTML = '<span class="btn-text">Calculate FD</span>';
        calculateBtn.disabled = false;
    }
}
...
function validateFDInputs() {
        ...
        if (!principal || principal <= 0 || principal < 1000) {
            showNotification('Please enter a valid principal amount (minimum ₹1,000)', 'error');
            return false;
        }

        if (!rate || rate <= 0 || rate > 20) {
            showNotification('Please enter a valid interest rate (0.1% - 20%)', 'error');
            return false;
        }

        if (!tenure || tenure <= 0 || tenure > 10) {
            showNotification('Please enter a valid tenure (1 - 10 years)', 'error');
            return false;
        }
